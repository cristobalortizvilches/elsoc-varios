...
knitr::opts_chunk$set(
  echo = FALSE,  # Oculta el código en todos los chunks
  warnings = FALSE
)
```
...
library(survey)
library(tidyverse)
load(url("https://dataverse.harvard.edu/api/access/datafile/6160173")) #2016-2021
load(url("https://dataverse.harvard.edu/api/access/datafile/10797987")) #2016-2023
```
...
datos.11.1 <- elsoc_long_2016_2023 %>% 
...
g11.1 <- datos.11.1 %>% 
  ...
  geom_text(position = position_stack(vjust = .5),
            size= 2.75, color = rep(c('white', 'white', 'black'), 7)) + 
...
elsoc_panel_m1 <- elsoc_long_2016_2023 %>% 
  filter(tipo_atricion == 1 & ola %in% c(1, 4, 5, 7)) %>% 
  group_by(idencuesta) %>% 
...
datos.11.2 <- data.frame((svytable(~clase_sub + ola + idencuesta, elsoc_panel_m1, round = F))) %>% 
  ...
  filter(Freq>0 & (ola == '2016'| ola == '2019' | ola == '2021' | ola == '2023')) %>%
  drop_na()
...
```{r ess-quintil, fig.align='center',fig.cap='Clase social subjetiva, según quintiles de ingreso (2023)'}
...
```{r esshijos-ess, fig.align='center',fig.cap='Si usted tiene actualmente hijos o si los tuviera en el futuro, ¿dónde cree usted que se ubicarían ellos?, según Clase social subjetiva (2023)'}
datos11.4 <- elsoc_long_2016_2023 %>% 
  filter(tipo_atricion == 1 & muestra == 1 & ola == 7 & 
...
datos.11.5 <- elsoc_long_2016_2023 %>% 
...
g11.5 <- 
  ...
  scale_y_continuous(labels = scales::percent) + 
  geom_text(position = position_stack(vjust = .5),
            size= 2.75, color = rep(c('black', 'white', 'white'), 7)) + 
  theme(legend.position = 'top',
        legend.title = element_blank())
g11.5
```
...
datos.11.6 <- elsoc_long_2016_2023 %>% 
  filter(tipo_atricion == 1 & muestra == 1 & 
...
datos.11.7 <- elsoc_long_2016_2023 %>% 
  filter(tipo_atricion == 1 & muestra == 1) %>%  
